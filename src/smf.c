...
/**
 * @brief Executes all entry actions from the direct child of topmost to the new
 * state
 *
...
static bool smf_execute_ancestor_run_actions(struct smf_ctx *ctx)
{
    ...
    /* The child state either transitioned or handled it. Either way, stop
     * propagating. */
...
/**
 * @brief Executes all exit actions from ctx->current to the direct child of
 * topmost
 *
 * @param ctx State machine context
...
int32_t smf_run_state(struct smf_ctx *const ctx)
{
    ...
    /* Executing a states run function could cause a transition, so clear
     * the internal state to ensure that the transition is handled
     * correctly.
     */
    smf_clear_internal_state(ctx);
#if CONFIG_SMF_ANCESTOR_SUPPORT == 1
